...
st.plotly_chart(px.bar(summary, x="country", y=temperature_column, labels=labels, title="Average Temperature of Sampled Observations", color_discrete_sequence=["steelblue"]), use_container_width=True)
...
st.plotly_chart(px.bar(counts, x="condition", y="count", labels=labels, title="Observations by Weather Condition", color_discrete_sequence=["darkorange"]), use_container_width=True)
